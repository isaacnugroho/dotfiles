...
local function init_theme()
  ...
  if not theme.gtk then
    debug.print_warning("Can't load GTK+3 theme. Using 'xresources' theme as a fallback.")
    return theme
  end
  ...
  local titlebar_font = RC.properties.titlebar_font or theme.font

  if (RC.properties.titlebar_font) then
    theme.titlebar_font_normal = RC.properties.titlebar_font
    theme.titlebar_font_focus = RC.properties.titlebar_font
  end

  theme.titlebar_height = beautiful.get_font_height(titlebar_font) * 1.2

  theme.titlebar_font_normal = titlebar_font
  ...
  theme.titlebar_fg_normal = choose_contrast_color(
      theme.titlebar_bg_normal, theme.gtk.menubar_fg_color, theme.gtk.menubar_bg_color)
  ...
  theme.titlebar_fg_focus = choose_contrast_color(
      theme.titlebar_bg_focus, theme.gtk.menubar_fg_color, theme.gtk.menubar_bg_color)
  ...
  theme.menu_height = theme.titlebar_height
...
function init()
  ...
  RC.functions.create_titlebar = titlebar.create_titlebar
  layouts.init()
  RC.main_menu = awful.menu({ items = RC.menu_items })
  mainbar.init()
end
